...
contract AddressDeployer {
    ...
    function deploy(bytes  memory _data) public onlyOwner returns(address addr) {
        // solium-disable-next-line security/no-inline-assembly
        assembly {
...
contract AddressToken is ERC721Full("AddressToken", "ATKN"), IAddressDeployerOwner {
    ...
    function deploy(uint256 _tokenId, bytes  memory _data) public returns(bool) {
        require(_isApprovedOrOwner(msg.sender, _tokenId));
        _burn(msg.sender, _tokenId);
    ...
    function bytecodeAt(address _addr) public view returns(bytes  memory outCode) {
        // solium-disable-next-line security/no-inline-assembly
        assembly {
    ...
    function addressToURI(address _addr) public pure returns(string memory) {
        ...
        str[10] = "x";
        for (uint i = 0; i < 20; i++) {
            str[11+i*2] = alphabet[uint8(value[i + 12] >> 4)];
            str[12+i*2] = alphabet[uint8(value[i + 12] & 0x0f)];
        }
        return string(str);
    }
    ...
    function firstAddressFromDeployer(address _deployer) public pure returns(address) {
        // solium-disable-next-line arg-overflow
        return address(uint160(uint256(keccak256(abi.encodePacked(byte(0xd6), byte(0x94), _deployer, byte(uint8(0x1)))))));
    }
}