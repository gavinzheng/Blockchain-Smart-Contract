...
contract FoodCart{
	...
	function addFoodItem (string _name, uint16 _price) public {
		foodItems[skuCount] = FoodItem({name: _name, sku: skuCount, price: _price, state: State.ForSale, foodItemExist: true});
	...
	hasBuyerPaidEnough(foodItems[_sku].price)
	{
		...
		emit Sold(foodItems[_sku].name, foodItems[_sku].sku, foodItems[_sku].price, uint8(foodItems[_sku].state), foodItems[_sku].foodItemExist);
	}
	...
	public 
	view 
	returns (string name, uint8 sku, uint16 price, uint8 state, bool foodItemExist){
